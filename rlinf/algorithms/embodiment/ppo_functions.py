...
def actor_critic_loss_fn(
    ...
    if torch.isnan(policy_loss):
        print("=" * 50)
        print("POLICY LOSS IS NaN - DEBUGGING")
        print("=" * 50)
        
        # Basic shapes and dtypes
        print(f"Shapes: logratio={logratio.shape}, advantages={advantages.shape}")
        print(f"Dtypes: logratio={logratio.dtype}, advantages={advantages.dtype}")
        print(f"Devices: logratio={logratio.device}, advantages={advantages.device}")
        
        # Check each component for NaN/Inf
        print("\n--- NaN/Inf Detection ---")
        print(f"logprobs has NaN: {torch.isnan(logprobs).any()}")
        print(f"logprobs has Inf: {torch.isinf(logprobs).any()}")
        print(f"old_logprobs has NaN: {torch.isnan(old_logprobs).any()}")
        print(f"old_logprobs has Inf: {torch.isinf(old_logprobs).any()}")
        print(f"logratio has NaN: {torch.isnan(logratio).any()}")
        print(f"logratio has Inf: {torch.isinf(logratio).any()}")
        print(f"ratio has NaN: {torch.isnan(ratio).any()}")
        print(f"ratio has Inf: {torch.isinf(ratio).any()}")
        print(f"advantages has NaN: {torch.isnan(advantages).any()}")
        print(f"advantages has Inf: {torch.isinf(advantages).any()}")
        print(f"surr1 has NaN: {torch.isnan(surr1).any()}")
        print(f"surr1 has Inf: {torch.isinf(surr1).any()}")
        print(f"surr2 has NaN: {torch.isnan(surr2).any()}")
        print(f"surr2 has Inf: {torch.isinf(surr2).any()}")
        
        # Statistics for each component
        print("\n--- Statistics ---")
        print(f"logprobs: min={logprobs.min():.6f}, max={logprobs.max():.6f}, mean={logprobs.mean():.6f}")
        print(f"old_logprobs: min={old_logprobs.min():.6f}, max={old_logprobs.max():.6f}, mean={old_logprobs.mean():.6f}")
        print(f"logratio: min={logratio.min():.6f}, max={logratio.max():.6f}, mean={logratio.mean():.6f}")
        print(f"ratio: min={ratio.min():.6f}, max={ratio.max():.6f}, mean={ratio.mean():.6f}")
        print(f"advantages: min={advantages.min():.6f}, max={advantages.max():.6f}, mean={advantages.mean():.6f}, std={advantages.std():.6f}")
        
        # Check for extreme values that could cause overflow
        print("\n--- Extreme Values Detection ---")
        extreme_logratio = torch.abs(logratio) > 10
        if extreme_logratio.any():
            print(f"Extreme logratio values (>10): {logratio[extreme_logratio]}")
            print(f"Corresponding logprobs: {logprobs[extreme_logratio]}")
            print(f"Corresponding old_logprobs: {old_logprobs[extreme_logratio]}")
        
        extreme_ratio = ratio > 100
        if extreme_ratio.any():
            print(f"Extreme ratio values (>100): {ratio[extreme_ratio]}")
        
        # Check clipping bounds
        print(f"\nClipping bounds: [{1 - clip_ratio_low:.3f}, {1 + clip_ratio_high:.3f}]")
        clipped_ratios = torch.clamp(ratio, 1 - clip_ratio_low, 1 + clip_ratio_high)
        print(f"Clipped ratios: min={clipped_ratios.min():.6f}, max={clipped_ratios.max():.6f}")
        
        # Check if the issue is in the final operations
        min_surr = torch.min(surr1, surr2)
        print(f"min(surr1, surr2): has_nan={torch.isnan(min_surr).any()}, has_inf={torch.isinf(min_surr).any()}")
        print(f"min(surr1, surr2): min={min_surr.min():.6f}, max={min_surr.max():.6f}")
        
        # Find first NaN location for detailed inspection
        if torch.isnan(min_surr).any():
            nan_idx = torch.isnan(min_surr).nonzero(as_tuple=True)[0][0]
            print(f"\nFirst NaN at index {nan_idx}:")
            print(f"  logratio[{nan_idx}] = {logratio[nan_idx]}")
            print(f"  ratio[{nan_idx}] = {ratio[nan_idx]}")
            print(f"  advantages[{nan_idx}] = {advantages[nan_idx]}")
            print(f"  surr1[{nan_idx}] = {surr1[nan_idx]}")
            print(f"  surr2[{nan_idx}] = {surr2[nan_idx]}")
            print(f"  min_surr[{nan_idx}] = {min_surr[nan_idx]}")
        
        # Check for zero/near-zero advantages that might interact badly with inf ratios
        near_zero_adv = torch.abs(advantages) < 1e-8
        if near_zero_adv.any():
            print(f"\nNear-zero advantages count: {near_zero_adv.sum()}")
            if torch.isinf(ratio[near_zero_adv]).any():
                print("WARNING: Infinite ratios with near-zero advantages!")
        
        print("=" * 50)
        raise NotImplementedError("Policy loss NaN detected - see debug info above")
