...
# Base log directory
BASE_LOG_DIR="${REPO_PATH}/logs/libero_spatial/full_ft"
...
for TASK_ID in $(seq 0 $((NUM_TASKS-1))); do
    ...
    OVERRIDES="env.fixed_task_ids=[${TASK_ID}] actor.seed=${BC_COEFF} actor.model.is_lora=False actor.model.partial_finetune=False actor.preallocate=0 cluster.num_nodes=2 actor.enable_offload=True rollout.enable_offload=True"
